...
ANTSSCRIPTNAME=~/bin/ANTS/antsIntroduction_short.sh
...
if [ "$RIGID" -eq 1 ] ;
    ...
    for IMG in $IMAGESETVARIABLE
      ...
      exe=" ${ANTSPATH}/ANTS $DIM -m MI[${TEMPLATE},${IMG},1,32] -o rigid_${IMG} -i 0 --use-Histogram-Matching --number-of-affine-iterations 1000x1000x1000x1000x1000 $RIGIDTYPE"
      ...
      if [ $DOQSUB -eq 1 ] ; then
		id=`qsub -N antsBuildTemplate_rigid -v ITK_GLOBAL_DEFAULT_NUMBER_OF_THREADS=1,LD_LIBRARY_PATH=$LD_LIBRARY_PATH,ANTSPATH=$ANTSPATH -l 'procs=1,mem=8gb,walltime=00:10:00' $qscript | awk '{print $1}'`
		jobIDs="$jobIDs $id"
		    sleep 0.5
...
# fi
  ...
  for IMG in $IMAGESETVARIABLE
    ...
    if [ $DOQSUB -eq 1 ]; then
	id=`qsub -N antsBuildTemplate_deformable_${i} -v ITK_GLOBAL_DEFAULT_NUMBER_OF_THREADS=1,LD_LIBRARY_PATH=$LD_LIBRARY_PATH,ANTSPATH=$ANTSPATH -l 'procs=1,mem=8gb,walltime=01:00:00' $qscript | awk '{print $1}'`
	     jobIDs="$jobIDs $id"
	     sleep 0.5

...
ANTS_MAX_ITERATIONS="100x70x50x20"
ANTS_TRANSFORMATION="SyN[0.1,3,0]"
ANTS_LINEAR_METRIC_PARAMS="1,32,Regular,0.25"
ANTS_LINEAR_CONVERGENCE="[1000x500x250x100,1e-6,10]"
ANTS_METRIC="CC"
  ...
  then
    ...
    if [[ ! -f ${EXTRACTION_INVERSE_WARP} ]];
      then
        ...
        ## Step 1 ##
          logCmd ${ANTSPATH}/ResampleImageBySpacing ${DIMENSION} ${EXTRACTION_TEMPLATE} ${EXTRACTION_INITIAL_AFFINE_FIXED} 0.2 0.2 0.2 1
          logCmd ${ANTSPATH}/ResampleImageBySpacing ${DIMENSION} ${N4_CORRECTED_IMAGES[0]} ${EXTRACTION_INITIAL_AFFINE_MOVING} 0.22 0.2 0.2 1
...
#             exe_initial_align="${ANTSPATH}/antsAffineInitializer ${DIMENSION} ${EXTRACTION_INITIAL_AFFINE_FIXED} ${EXTRACTION_INITIAL_AFFINE_MOVING} ${EXTRACTION_INITIAL_AFFINE} 15 0.1 0 10"
 ...
 #         exe_initial_align="${exe_initial_align} -g [40]"
